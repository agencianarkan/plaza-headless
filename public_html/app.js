        ...
        if (auth.checkAuth()) {
            try {
                ...
                if (usuario) {
                    ...
                    if (userInfo) {
                        userInfo.textContent = `Usuario: ${usuario.nombre || usuario.email}`;
                    }
                }
            } catch (error) {
...
async function checkAndShowShippingMenu() {
    ...
    // Por ahora mostramos el menú siempre
    // En el futuro puedes agregar lógica para verificar permisos desde el backend
    try {
        // Intentar verificar permisos haciendo una petición de prueba
        // Si falla, ocultamos el menú
